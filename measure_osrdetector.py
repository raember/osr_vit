...
def main(opt, model):
    ...
    if opt.in_dataset == "CUB":
        ...
        if opt.in_dataset == "MNIST" or opt.in_dataset == "SVHN" or opt.in_dataset == "CIFAR10":
            ...
            total_classes = 8
            assert opt.leave_out_class >= 0
            known_classes = list(range(total_classes))
            known_classes.remove(opt.leave_out_class)
...
def run_ood_distance(opt):
    ...
    for dir in os.listdir(experiments_dir):
        ...
        if len(dir_split) != 10:
            continue
        exp_name, dataset, model_arch, _, _, _, num_classes, random_seed, _, _ = dir_split
        opt = eval("get_{}_config".format(model_arch))(opt)
        if exp_name == 'osrdetector' and dataset == opt.in_dataset and opt.in_num_classes == int(num_classes[2:]):
            ...
            if opt.leave_out_class != orig_config['leave_out_class']:
                continue
            print(f"Found experiment for LOC {opt.leave_out_class}")
            ...
            for ckpt_file in os.listdir(ckpt_dir):
                if ckpt_file.endswith(".pth"):
                    print(f"Measuring checkpoint {ckpt_file}")
